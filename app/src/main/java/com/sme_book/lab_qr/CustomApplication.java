...
// 카카오 로그인 api를 정상적으로 사용할 수 있게끔 하는 작업
        ...
        public ISessionConfig getSessionConfig() {
            ...
            return new ISessionConfig() {
                ...
                public boolean isUsingWebviewTimer() {
                    ...
                    // true 를 리턴할경우 webview로그인을 사용하는 화면서 모든 webview에 onPause와 onResume 시에 Timer를 설정해 주어야 함
                    // 지정하지 않을 시 false로 설정
                }
                ...
                public ApprovalType getApprovalType() {
                    ...
                    // 일반 사용자가 아닌 Kakao와 제휴된 앱에서만 사용되는 값으로, 값을 채워주지 않을경우 ApprovalType.INDIVIDUAL 값을 사용하게 됨
                }

                @Override
                public boolean isSaveFormData() {
                    return true;
                    // Kakao SDK 에서 사용되는 WebView에서 email 입력폼의 데이터를 저장할지 여부를 결정
                    // true일 경우, 다음번에 다시 로그인 시 email 폼을 누르면 이전에 입력했던 이메일이 나타남
                }
            };
        }
